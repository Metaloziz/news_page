...
export const CommentForm: FC<CommentFormPropsType> = memo(({addComment}) => {
  ...
  return <div>Ваш комментарий:
    <form onSubmit={handleSubmit(onSubmit)}>
      <div><input defaultValue="boss" {...register("author")} placeholder={'автор'}/>
      </div>
      {errors.text && <span>This field is required</span>}
      <div><input {...register("text", {required: true})}
                  defaultValue="какой хороший сайт"
                  placeholder={'комментарий'} maxLength={300}/></div>
      {errors.text && <span>This field is required</span>}
